...
订单/成交/仓位三类订阅全部成功且最近心跳不超过 30 秒时为 `true`。Binance、OKX、Bybit 与 Bitget
的认证连接已装配到常驻 worker；Gate/MEXC 仍会以该字段为 `false` 保持阻断。已接入的私有事件当前用于连接健康确认，
...
保护价，并生成满足 OKX 纯字母数字、Gate `t-` 前缀及其他交易所长度约束的客户端订单 ID。该能力尚未
开放 HTTP 入口；在预览、普通二次确认和账户就绪判定完成前，前端不能触发真实计划。内部真实执行器已
