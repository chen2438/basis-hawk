...
配对持仓、交易账本、内部划转、自动策略、审计与通知均为一级页面，窄屏时菜单收拢为图标栏。页面直接
读取下列 API 真相源，而不是复制内存状态：
执行页展示全局与逐账户对账、私有流、挂单/仓位/成交计数，并提供普通确认后的暂停和重新对账；
`blocked`、`paused` 或 `ready` 都可请求全量重新对账，只有已经处于 `reconciling` 时暂时禁用；账户页
管理加密凭据且只展示掩码，可按需读取余额和权限；持仓页列出配对仓位；交易账本页列出最近 100 条
交易意图、订单腿、成交和已实现 PnL；划转页只接受同所 USDT
...
锁定分支或在发现新版本后显式确认更新；更新期间页面会轮询脱敏状态，服务短暂离线属于预期行为。
...
- `GET /api/trades/intents`：按创建时间倒序读取交易意图及双腿，允许精确 `status` 过滤。
...
- `POST /api/trades/open/preview`：为已配置凭据的 `sandbox`/`live` 账户生成 15 秒真实开仓预览票据，
  ...
  交易规则预览错误也由前端映射为中文。
  确认前若价格、保护价、数量、交易规则或配置指纹变化，或者 15 秒票据过期，后端继续拒绝旧票据；
  前端显示中文重新预览说明并立即移除失效的开仓/平仓票据。订单簿重新读取产生的新采集时间不参与
  指纹，交易条件完全相同时可以确认，不会因时间戳必然变化而永久拒绝。
...
界面使用简体中文、深色固定侧栏与高对比度浅色数据主区，API 字段、代码枚举和交易所名保持英文。
正收益、健康状态和当前选中机会使用绿色语义，负收益使用红色，次级说明使用深灰色，保证浅色主区
上的可读性。费用仅是可编辑估算，页面必须提示地区、账户等级和活动费率可能不同。
