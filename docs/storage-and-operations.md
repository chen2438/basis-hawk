...
永续订单提交前必须确认任务腿请求的保证金模式、杠杆和账户持仓模式。Binance 的保证金模式是逐合约
配置，账户快照不能代表目标空仓合约，因此 Cross 腿允许进入逐合约配置流程，并在发单前通过
`symbolConfig` 同时回读 `CROSSED` 和请求杠杆；模式切换前若目标合约已有活动订单或持仓则阻断。
OKX Cross 腿以订单 `tdMode` 选择模式，并以目标 SWAP 的 `mgnMode=cross` 配置和回读杠杆；调整前
同样要求目标无挂单或持仓。Gate portfolio 快照已确认共享跨仓，只允许 1x。其他交易所 Cross 腿
保持失败关闭。
...
机会聚合器从每所单行 JSON 最新行情缓存按基础资产分组，不新增高频表。资金费/基差候选对每个可用
现货场所和永续场所做笛卡尔组合；跨所资金费候选只保留不同交易所且空腿日 funding 高于多腿的组合。
