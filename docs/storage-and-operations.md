...
worker 定期写入 `account_snapshots` 和各账户最新 `account_reconciliation` 状态；全局
`execution_control` 在对账开始时进入 `reconciling`。目前仅余额、权益和模式已接入持久化，
挂单、成交与仓位尚未完成，因此每轮结束都会保持 `blocked`，不能据此执行交易。
