...
Gate Sandbox 的资金费率、历史 APR 和初步候选仍来自常规公共行情扫描，但实际开平仓价格与容量必须
来自 Gate TestNet。worker 会先读取 TestNet 标的目录，只允许 TestNet 同时存在的标的进入自动执行，
并用 TestNet 自己的价格/数量步长、最小数量/名义额及合约乘数生成并固化订单腿；目录、规则或订单簿
不可用时跳过。自动补偿同样重新读取 TestNet 目录和盘口，不得用实盘规则或深度为沙盒订单背书，也不得
跨环境拼腿。
