...
私有交易统一限定为限价 IOC。当前 Binance、OKX、Bybit、Bitget Classic V2 及 Gate LIVE 适配器已
实现现货及 USDT 永续下单和按交易所/客户端订单 ID 撤单；永续仅允许“卖出开空”或“reduce-only
买回空仓”。
单向模式显式发送
...
后的十进制张数发送，卖出开空为负、买入平空为正，并显式发送 `X-Gate-Size-Decimal: 1`。下单与撤单
