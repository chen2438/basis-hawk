...
私有交易统一限定为限价 IOC。当前 Binance 适配器已实现现货及 USDT 永续下单和按交易所/客户端订单 ID
撤单；永续仅允许“卖出开空”或“reduce-only 买回空仓”。单向模式显式发送 `reduceOnly`，双向模式按
Binance 规则发送 `positionSide=SHORT` 且不发送该所禁止的 `reduceOnly` 参数，但本地仍保留
reduce-only 语义。订单响应只证明交易所接受或返回当前状态，最终成交仍以私有成交流或 REST 成交明细
